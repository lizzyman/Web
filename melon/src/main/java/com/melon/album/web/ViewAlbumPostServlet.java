...
public class ViewAlbumPostServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		AlbumVO albumVO = albumBiz.getOneAlbum(albumId);
		
		String postPath = "D:\\uploadFiles\\post\\";
		postPath += albumVO.getArtistId() + File.separator;
		postPath += albumVO.getAlbumName();

		DownloadUtil downloadUtil = DownloadUtil.getInstance(postPath);
		downloadUtil.download(request, response, albumVO.getPost(), albumVO.getPost());
	}
